...
#define  __MQTT_GLOAL     //__MQTT_GLOAL宏定义
...
{	
	...
	// MQTT_SubscribeQs0();	                          //发送缓冲区添加订阅topic，等级0	
	// LED_State();                                      //发送缓冲区添加 开关当前状态 发给服务器
...
/*----------------------------------------------------------*/
/*函数名：SUBSCRIBE订阅topic报文 等级0                      */
/*参  数：topic_buff：订阅topic报文的缓冲区                 */
/*返回值：无                                                */
/*----------------------------------------------------------*/
...
{	
	...
	Fixed_len = 1;                              //固定报头长度暂时先=1
	Variable_len = 2;                           //可变报头长度=2
	Payload_len = 0;                            //先清除Payload_len变量
	for(i=0;i<S_TOPIC_NUM;i++)                  //循环统计topic字符串长度
		Payload_len += strlen(Stopic_Buff[i]);  //每次累加
	Payload_len += 3*S_TOPIC_NUM;               //负载长度不仅包含topic字符串长度，还有等级0的标识字节，一个topic一个，以及2个字节的字符串长度标识字节，一个topic三个，所以再加上3*S_TOPIC_NUM    
	Remaining_len = Variable_len + Payload_len; //剩余长度=可变报头长度+负载长度

	temp_buff[0]=0x82;                          //固定报头第1个字节 ：固定0x82		
	do{                                         //循环处理固定报头中的剩余长度字节，字节量根据剩余字节的真实长度变化
		temp = Remaining_len%128;               //剩余长度取余128
		Remaining_len = Remaining_len/128;      //剩余长度取整128
		if(Remaining_len>0)               	
	...
	temp_buff[Fixed_len+0]=0x00;                //报文标识符高字节，固定使用0x00
	temp_buff[Fixed_len+1]=0x01;		        //报文标识符低字节，固定使用0x01
	
	len = 0;
	for(i=0;i<S_TOPIC_NUM;i++){                                                     //循环复制负载数据		
		temp_buff[Fixed_len+2+len]=strlen(Stopic_Buff[i])/256;                      //topic字符串 长度高字节 标识字节
		temp_buff[Fixed_len+3+len]=strlen(Stopic_Buff[i])%256;		                //topic字符串 长度低字节 标识字节
		memcpy(&temp_buff[Fixed_len+4+len],Stopic_Buff[i],strlen(Stopic_Buff[i]));  //复制topic字串		
		temp_buff[Fixed_len+4+strlen(Stopic_Buff[i])+len]=0;                        //订阅等级0	
		len += strlen(Stopic_Buff[i]) + 3;                                          //len等于本次循环中添加的数据量 等于 topic字串本身长度 + 2个字节长度标识 + 1个字节订阅等级
	}
...
/*----------------------------------------------------------*/
/*函数名：DISCONNECT报文，断开连接                          */
/*参  数：无                                                */
/*返回值：无                                                */
/*----------------------------------------------------------*/
void MQTT_DISCONNECT(void)
{
	temp_buff[0]=0xE0;              //第1个字节 ：固定0xE0                      
	temp_buff[1]=0x00;              //第2个字节 ：固定0x00 
