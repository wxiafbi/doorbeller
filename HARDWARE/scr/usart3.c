...
unsigned int Usart2_RxCounter = 0;      // 定义一个变量，记录串口3总共接收了多少字节的数据
char Usart2_RxBuff[USART2_RXBUFF_SIZE]; // 定义一个数组，用于保存串口3接收到的数据
#endif
...
#endif
    USART_Init(USART2, &USART_InitStructure); // 设置串口3

#if USART2_RX_ENABLE                                                    // 如果使能接收模式
    USART_ClearFlag(USART2, USART_FLAG_RXNE);                           // 清除接收标志位
    USART_ITConfig(USART2, USART_IT_RXNE, ENABLE);                      // 开启接收中断
    NVIC_InitStructure.NVIC_IRQChannel                   = USART2_IRQn; // 设置串口3中断
...
void Usart2_IDELInit(unsigned int bound)
{
    ...
    NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2); // 设置中断向量分组：第2组 抢先优先级：0 1 2 3 子优先级：0 1 2 3
#endif
    USART_DeInit(USART2);                                  // 串口3 所有寄存器 恢复默认值
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART2, ENABLE); // 使能串口3时钟
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);  // 使能GPIOB时钟
    USART_DeInit(USART2);                                  // 串口3寄存器重新设置为默认值
    GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_2;            // 准备设置PB10
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;      // IO速率50M
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_AF_PP;       // 复用推挽输出，用于串口3的发送
    GPIO_Init(GPIOA, &GPIO_InitStructure);                 // 设置PB10
    ...
    NVIC_Init(&NVIC_InitStructure);                                     // 设置串口3中断
#endif
    USART_DMACmd(USART2, USART_DMAReq_Rx, ENABLE); // 开启串口3 DMA接收
    USART_DMACmd(USART2, USART_DMAReq_Tx, ENABLE); // 开启串口3 DMA发送
...
void Usart2DMA_init(void)
{
    ...
    DMA_Initstructure.DMA_BufferSize         = USART2_RXBUFF_SIZE;          // 大小设置为串口3的缓冲区大小
    ...
    DMA_Init(DMA1_Channel6, &DMA_Initstructure);                            // 设置通道3
    DMA_Cmd(DMA1_Channel6, ENABLE);                                         // 使能通道3
    ...
    DMA_Initstructure.DMA_M2M                = DMA_M2M_Disable;                       // 没有设置为内存到内存传输
    DMA_Init(DMA1_Channel7, &DMA_Initstructure);                                      // 设置通道2
    DMA_Cmd(DMA1_Channel7, DISABLE);                                                  // 先不要使能通道2

    DMA_ClearITPendingBit(DMA1_IT_TC7);                                        // 清除DMA1通道2 发送完成中断标志
    DMA_ITConfig(DMA1_Channel7, DMA_IT_TC, ENABLE);                            // 开启DMA1通道2 发送完成中断
